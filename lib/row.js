...
Row.prototype.put = function(columns, values, callback) {
  ...
  if (args.length > 2) {
    ...
    url = utils.url.encode({
      table: this.table,
      key: this.key || "___node-client-false-row-key___",
      columns: columns
    });
  } else {
    ...
    data.forEach((d) => {
      var key;
      key = d.key || this.key;
      key = utils.base64.encode(key, this.client.options.encoding);
      if (!(key in cellsKeys)) {
        cellsKeys[key] = [];
      }
    ...
    url = utils.url.encode({
      table: this.table,
      key: "___node-client-false-row-key___",
      columns: ['test:']
    });
  }
